...
function smProcessMeta(&$buffer) {
    ...
    if (preg_match_all('~<meta.*?name=[\'\"]+([^\'\"]+)[\'\"]+[^>]*>~', $buffer, $metatags)) {
        ...
        foreach($allowed_meta as $meta) {
            ...
            $key = array_search($meta['@attributes']['name'], $metatags[1]);
            // Если есть замена
            if ($rule && isset($rule[$meta['@attributes']['name']])) {
                $content = $rule[$meta['@attributes']['name']];
            }
            // Если есть на странице
            elseif ($key !== false) {
                preg_match('~content=[\'\"]+([^\'\"]+)[\'\"]+~i', $metatags[0][$key], $m);
                $content = $m[1];
            }
            ...
            $block_meta[] = '<meta name="'.$meta['@attributes']['name'].'" content="'.$content.'"'.$close_tag;
        }
    }
    // Добавим кодировку
    $block_meta[] = '<meta http-equiv="content-type" content="text/html; charset='.SEOMOD_CHARSET.'"'.$close_tag;
    // Старые метатеги нам не нужны
    $buffer = preg_replace('~<meta.*?(name|http-equiv)=.?([\w]+).?[^>]*>~', '', $buffer);
...
function smReplace(&$buffer, &$rule) {
    if (isset($rule['@attributes']['regexp']) && $rule['@attributes']['regexp'] == '1') {
        $buffer = preg_replace('~'.addslashes($rule['find']).'~is', $rule['replace'], $buffer);
    } else {
...
function smProcessLinks(&$buffer) {
    ...
    if (!is_null($rules)) {
        ...
        if (preg_match_all('~href=([\'\"]){1}(.*?)[\'\"]{1}~mi', $buffer, $match_links)) {
            // Пройдемся по всем ссылкам на сайте
            foreach ($match_links[2] as $key => $link) {
                $delim = $match_links[1][$key];
...
function smCheckExcept($url) {
    ...
    if (isset($options['except'])) {
        ...
        if (isset($url[1])) {
            ...
            if (count(array_diff($excepts, $get)) != count($excepts)) {
                return false;
            }
        }
    }
